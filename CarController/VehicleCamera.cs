...
public class VehicleCamera : MonoBehaviour
{
    ...
    private void Awake() {  // If you will choose the car in garage scene please mark the isAuto variable in the inspector of camera
        Debug.Log(GameObject.FindGameObjectsWithTag("Player").Length+"   Tane Player Elemanı var");
        if(isAuto && GameObject.FindGameObjectsWithTag("Player").Length==1){
    ...
    public void FindCar(){
        ...
        if(GameObject.FindGameObjectsWithTag("Player").Length==1){
            target=GameObject.FindGameObjectWithTag("Player").transform;
            GameObject.FindGameObjectWithTag("Minimap").GetComponent<MinimapCamera>().target=target;
        }
            
        
    }
    public void OnClick(){
        target.GetComponent<skillController>().OnClick();
    }
    ...
    public void RestCar()
    {
        ...
        if (restTime == 0 && carScript.speed<1.0f)
        {
            myRigidbody.AddForce(Vector3.up * 500000);
            myRigidbody.MoveRotation(Quaternion.Euler(0, transform.eulerAngles.y, 0));
            restTime = 2.0f;
            target.transform.position=GameObject.Find("Instantiate").GetComponent<InstantiateExample>().GiveAStartPosition().position;
